...
""")
    ...
    # Check workspace information includes BDT version
    assert "bdt_version" in status["workspace"]
    assert status["workspace"]["bdt_version"] == "0.1.0"
...
@patch('subprocess.run')
def test_get_package_version_formats(mock_subprocess, workspace_manager, temp_workspace):
