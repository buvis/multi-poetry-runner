...
@click.option('--check-permissions', is_flag=True, 
...
@click.pass_context
def workspace_add_repo(ctx: click.Context, repo_url: str, name: Optional[str],
                      depends: Optional[str], branch: str) -> None:
...
@click.option('--check-transitive', is_flag=True, help='Analyze transitive dependencies')
@click.pass_context
def deps_status(ctx: click.Context, verbose: bool, check_transitive: bool) -> None:
    ...
    try:
        ...
        manager.display_status(status, verbose=verbose, show_transitive=check_transitive)
    except Exception as e:
...
@click.option('--alpha', is_flag=True, help='Create alpha version (e.g., 0.1.6-alpha.1)')
@click.option('--dry-run', is_flag=True, help='Show what would be changed without making changes')
@click.option('--update-dependents', is_flag=True, default=True, help='Update dependent repositories')
@click.option('--dependents-bump', type=click.Choice(['patch', 'minor', 'major']), default='patch',
              help='Bump type for dependent repositories (default: patch)')
@click.option('--validate', is_flag=True, default=True, help='Run validation tests after version bump')
@click.pass_context
def version_bump(ctx: click.Context, repository: str, bump_type: str, alpha: bool, 
                dry_run: bool, update_dependents: bool, dependents_bump: str, validate: bool) -> None:
    ...
    try:
        ...
        if success:
            ...
            console.print(f"[green]✓ {action} {repository} version ({bump_type}{' alpha' if alpha else ''})[/green]")
        else:
...
@click.option('--show-dependents', is_flag=True, help='Show which repositories depend on each package')
@click.pass_context
def version_status(ctx: click.Context, repository: Optional[str], show_dependents: bool) -> None:
            ...
            if repo.path.exists():
                ...
                if repo.dependencies:
                    ...
                    for dep_name in repo.dependencies:
                        ...
                        if dep_repo:
                            console.print(f"    {dep_name} -> package: {dep_repo.package_name}")
                        else:
                            console.print(f"    {dep_name} -> [red]not found in config[/red]")
...
def version_sync(ctx: click.Context, dry_run: bool, force: bool) -> None:
    ...
    try:
        ...
        if success:
            ...
            console.print(f"[green]✓ {action} dependency versions across all repositories[/green]")
        else:
...
@click.option('--stage', type=click.Choice(['dev', 'rc', 'prod']), 
              required=True, help='Release stage')
@click.option('--repositories', help='Comma-separated list of repositories to release (default: all)')
@click.option('--version', help='Default version for all repositories')
@click.option('--repo-versions', help='JSON string mapping repository names to versions')
...
@click.pass_context
def release_create(ctx: click.Context, stage: str, repositories: Optional[str],
                  version: Optional[str], repo_versions: Optional[str],
                  dry_run: bool, skip_tests: bool,
                  force: bool, parallel: bool) -> None:
    ...
    try:
        ...
        if repo_versions:
            ...
            try:
                ...
                console.print(f"[red]Invalid JSON in --repo-versions: {repo_versions}[/red]")
                sys.exit(1)
...
@click.option('--parallel', is_flag=True, help='Run tests in parallel')
@click.option('--environment', type=click.Choice(['local', 'docker']),
              default='local', help='Test environment')
@click.option('--junit-output', is_flag=True, help='Generate JUnit XML output')
@click.pass_context
def test_integration(ctx: click.Context, parallel: bool, environment: str,
                     junit_output: bool) -> None:
