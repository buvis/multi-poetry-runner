...
"""
    ...
    def setup_workspace(self, ci_mode: bool = False) -> None:
        ...
        with Progress(
            ...
            # Clone repositories
            task = progress.add_task("Cloning repositories...", total=len(config.repositories))
            ...
            task = progress.add_task("Setting up environments...", total=len(config.repositories))
    ...
    def _clone_repository(self, repo: RepositoryConfig) -> None:
        ...
        if repo.path.exists():
            logger.info(f"Repository {repo.name} already exists, pulling latest changes")
            subprocess.run(
    ...
    def get_status(self, check_permissions: bool = False) -> Dict[str, Any]:
        ...
        for repo in config.repositories:
            ...
            if repo.path.exists():
                ...
                try:
                    ...
                    repo_status["git_status"] = "clean" if not result.stdout.strip() else "dirty"
        ...
        if marker_file.exists():
            status["workspace"]["dependency_mode"] = marker_file.read_text().strip().split('\n')[0]
        else:
