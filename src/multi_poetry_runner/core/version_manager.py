...
class VersionManager:
    ...
    def bump_version(self, repository: str, bump_type: str, alpha: bool = False,
                    dry_run: bool = False, update_dependents: bool = True,
                    dependents_bump: str = 'patch', validate: bool = True) -> bool:
        ...
        if not repo:
            console.print(f"[red]Repository '{repository}' not found in configuration[/red]")
            return False
        ...
        try:
            ...
            if not current_version:
                console.print(f"[red]Could not determine current version for {repository}[/red]")
                return False
            
            new_version = self._calculate_new_version(current_version, bump_type, alpha)
            console.print(f"Version bump: [yellow]{current_version}[/yellow] → [green]{new_version}[/green]")
            ...
            if not dry_run:
                ...
                console.print(f"[green]✓[/green] Updated {repository} to version {new_version}")
            else:
                console.print(f"[dim]Would update {repository} to version {new_version}[/dim]")
            ...
            if update_dependents:
                ...
                if dependents:
                    ...
                    console.print(f"  [dim]Dependents will get {dependents_bump}{' alpha' if alpha else ''} version bump[/dim]")
                    ...
                    for dependent_repo in dependents:
                        ...
                        if not dry_run:
                            ...
                            dep_update_success = self._update_dependency_version(dependent_repo, repo.package_name, new_version)
                            
                            if dep_update_success:
                                # Then bump the dependent repository's own version
                                dependent_current_version = self._get_current_version(dependent_repo)
                                if dependent_current_version:
                                    ...
                                    try:
                                        self._update_repository_version(dependent_repo, dependent_new_version)
                                        ...
                                        console.print(f"  [green]✓[/green] Updated {dependent_repo.name}: {dependent_current_version} → {dependent_new_version}")
                                    except Exception as e:
                                        console.print(f"  [red]✗[/red] Failed to bump version for {dependent_repo.name}: {e}")
                                else:
                                    console.print(f"  [yellow]?[/yellow] Could not determine current version for {dependent_repo.name}")
                            else:
                                console.print(f"  [red]✗[/red] Failed to update dependency in {dependent_repo.name}")
                        else:
                            ...
                            if dependent_current_version:
                                ...
                                console.print(f"  [dim]Would update {dependent_repo.name}: {dependent_current_version} → {dependent_new_version}[/dim]")
                            else:
                                console.print(f"  [dim]Would update {dependent_repo.name} (could not determine current version)[/dim]")
                else:
                    console.print("[dim]No dependent repositories found[/dim]")
            
            # Step 4: Record version history
            if not dry_run:
                self._record_version_history(repository, current_version, new_version, 
                                           bump_type, alpha, dependents_updated)
            
            # Step 5: Run validation tests
            if validate and not dry_run:
                console.print(f"\n[bold]Running validation tests...[/bold]")
                validation_success = self._run_validation_tests(repo, dependents_updated)
            ...
            if dependents_updated:
                ...
                for dep in dependents_updated:
                    console.print(f"    {dep['name']}: {dep['old_version']} → {dep['new_version']} ({dep['bump_type']})")
    ...
    def _calculate_new_version(self, current_version: str, bump_type: str, alpha: bool = False) -> str:
    ...
    def _update_dependency_version(self, dependent_repo: RepositoryConfig, 
        ...
        try:
            ...
            if found_dependency is None:
                logger.warning(f"Dependency {package_name} (or variations) not found in {dependent_repo.name}")
    ...
    def _record_version_history(self, repository: str, old_version: str, new_version: str,
                              bump_type: str, alpha: bool, dependents_updated: List[Dict[str, str]]) -> None:
    ...
    def _run_validation_tests(self, repo: RepositoryConfig, dependents_updated: List[Dict[str, str]]) -> bool:
        ...
        def _test_repository(repo_config: RepositoryConfig, repo_name: str) -> bool:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    console.print(f"  [yellow]?[/yellow] No tests found for {repo_name} (OK)")
                    return True
                else:
                    console.print(f"  [red]✗[/red] Tests failed for {repo_name}")
                    logger.error(f"Test output for {repo_name}: {result.stdout}\n{result.stderr}")
                    return False
                ...
                console.print(f"  [yellow]?[/yellow] pytest not found for {repo_name} (OK)")
                return True
            ...
            for dep_name, dep_spec in dependencies.items():
                ...
                if dep_repo:
                    info = {
                        "name": dep_name,
                        "managed": True,
                        "current_version": self._get_current_version(dep_repo) if dep_repo.path.exists() else None
                    }
                    ...
                    # Check compatibility
                    if info["current_version"] and info["required_version"] and not info["is_path"]:
    ...
    def display_version_status(self, status: Dict[str, Any], show_dependents: bool = False) -> None:
        ...
        for repo in status["repositories"]:
            ...
            version_display = f"[green]{version}[/green]" if version != "N/A" else "[dim]N/A[/dim]"
            ...
            if deps:
                ...
                incompatible_count = len([d for d in managed_deps if d.get("compatible") is False])
                
                if incompatible_count > 0:
                    deps_display = f"[red]{len(managed_deps)} ({incompatible_count} issues)[/red]"
                else:
            ...
            if show_dependents:
                ...
                dependents_display = ", ".join(dependents) if dependents else "[dim]None[/dim]"
        ...
        for repo in status["repositories"]:
            ...
            incompatible_deps = [d for d in dependencies if d.get("compatible") is False]
            
            if incompatible_deps:
                console.print(f"\n[red bold]Dependency Issues in {repo['name']}:[/red bold]")
                for dep in incompatible_deps:
                    console.print(f"  [red]•[/red] {dep['name']}: requires {dep['required_version']}, but current is {dep['current_version']}")
        ...
        if history:
            ...
            for entry in reversed(history[-5:]):  # Show last 5 entries
                ...
                change_str = f"{old_ver} → {new_ver} ({bump_type}{'α' if alpha else ''})"
                
                dependents = entry.get("dependents_updated", [])
                dependents_str = ", ".join(dependents) if dependents else "[dim]None[/dim]"
                
                history_table.add_row(date_str, repo_name, change_str, dependents_str)
            
            console.print(history_table)
    
    def sync_dependency_versions(self, dry_run: bool = False, force: bool = False) -> bool:
        ...
        if not updates_needed:
            console.print("[green]✓ All dependency versions are already synchronized[/green]")
            return True
        ...
        for update in updates_needed:
            console.print(f"  {update['repo'].name}: {update['dependency']} "
                         f"{update['current_required']} → {update['new_required']}")
        
        if not force and not dry_run:
            if not console.input("\nProceed with updates? [y/N]: ").lower().startswith('y'):
        ...
        for update in updates_needed:
            if not dry_run:
                ...
                if success:
                    ...
                    console.print(f"  [red]✗[/red] Failed to update {update['repo'].name}")
            else:
