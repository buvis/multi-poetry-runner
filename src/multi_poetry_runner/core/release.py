...
class ReleaseCoordinator:
    ...
    def create_release(self, stage: str, repositories: Optional[List[str]] = None,
                      version: Optional[str] = None, repository_versions: Optional[Dict[str, str]] = None,
                      dry_run: bool = False, skip_tests: bool = False,
                      force: bool = False, parallel: bool = False) -> bool:
        ...
        logger.info(f"Starting {stage} release for repositories: {[r.name for r in repos_to_release]}")
        ...
        if repository_versions:
            for repo_name in repository_versions:
                if not any(r.name == repo_name for r in repos_to_release):
                    logger.error(f"Repository '{repo_name}' specified in versions but not in release list")
                    return False
        ...
        release_order = [name for name in self.config_manager.get_dependency_order() 
        ...
        if parallel and stage == "dev":
            ...
            success = self._process_repositories_parallel(
                release_order, release_stage, repos_to_release, version, repository_versions,
                dry_run, skip_tests
            )
        else:
            success = self._process_repositories_sequential(
                release_order, release_stage, repos_to_release, version, repository_versions,
                dry_run, skip_tests, force
            )
    ...
    def _process_repositories_sequential(self, release_order: List[str],
                                       stage: ReleaseStage, repos_to_release: List[RepositoryConfig],
                                       version: Optional[str], repository_versions: Optional[Dict[str, str]],
                                       dry_run: bool, skip_tests: bool, force: bool) -> bool:
        ...
        with Progress(
            ...
            task = progress.add_task("Releasing repositories...", total=len(release_order))
            ...
            for repo_name in release_order:
                ...
                if not success:
                    if force:
                        logger.warning(f"Repository {repo_name} failed but continuing due to --force")
    ...
    def _process_repositories_parallel(self, release_order: List[str],
                                     stage: ReleaseStage, repos_to_release: List[RepositoryConfig],
                                     version: Optional[str], repository_versions: Optional[Dict[str, str]],
                                     dry_run: bool, skip_tests: bool) -> bool:
    ...
    def _process_single_repository(self, repo: RepositoryConfig, stage: ReleaseStage,
                                 version: Optional[str], dry_run: bool,
                                 skip_tests: bool) -> bool:
        ...
        try:
            ...
            if stage == ReleaseStage.PROD and not dry_run:
                if self._version_already_released(repo, new_version):
                    logger.warning(f"Version {new_version} has already been released for {repo.name}")
                    console.print(f"[yellow]Version {new_version} already exists for {repo.name}, skipping[/yellow]")
    ...
    def _determine_version(self, repo: RepositoryConfig, stage: ReleaseStage,
    ...
    def _update_lock_file(self, repo: RepositoryConfig) -> None:
        ...
        try:
            ...
            logger.warning(f"Standard poetry lock failed for {repo.name}, trying --regenerate")
            try:
                ...
                logger.error(f"Both standard and regenerate lock failed for {repo.name}")
                raise regenerate_error
        except subprocess.TimeoutExpired:
            logger.warning(f"Poetry lock timed out for {repo.name}, trying --regenerate")
            try:
                subprocess.run(
                    ["poetry", "lock", "--regenerate"],
                    cwd=repo.path,
                ...
                logger.info(f"Successfully regenerated lock file for {repo.name} after timeout")
            except subprocess.CalledProcessError as regenerate_error:
    ...
    def _tag_release(self, repo: RepositoryConfig, version: str) -> None:
        ...
        try:
            ...
            if result.stdout.strip():
                logger.warning(f"Tag {tag} already exists in {repo.name}, skipping tag creation")
                return
        except subprocess.CalledProcessError:
            logger.warning(f"Could not check for existing tags in {repo.name}")
    ...
    def _update_dependent_repositories(self, released_repos: List[RepositoryConfig]) -> bool:
        ...
        ordered_dependents = [repo for repo_name in dependency_order 
                            for repo in all_dependents if repo.name == repo_name]
        
        # Track which repositories have been updated in this release cycle
        updated_in_this_cycle = {repo.name: self._get_current_version(repo) for repo in released_repos}
        success_count = 0
            ...
            try:
                ...
                if dependencies_updated:
                    ...
                    if current_dep_version:
                        new_dep_version = self._calculate_dependent_version_bump(current_dep_version)
                        self._update_repository_version(dep_repo, new_dep_version)
                        console.print(f"    Bumped {dep_repo.name} version: {current_dep_version} → {new_dep_version}")
                    ...
                    if result.stdout.strip():
                        ...
                        for released_repo in released_repos:
                            if released_repo.name in updated_in_this_cycle:
                                updated_deps.append(f"{released_repo.package_name}@{updated_in_this_cycle[released_repo.name]}")
                        
                        # Add other cascading updates
                        for updated_repo_name, updated_version in updated_in_this_cycle.items():
                            if updated_repo_name not in [r.name for r in released_repos] and updated_repo_name != dep_repo.name:
                                updated_repo = self.config_manager.get_repository(updated_repo_name)
                                if updated_repo and updated_repo_name in dep_repo.dependencies:
                                    updated_deps.append(f"{updated_repo.package_name}@{updated_version}")
                        ...
                        commit_message = f"Update dependencies: {', '.join(unique_deps)}"
                        ...
                        console.print(f"  [green]✓[/green] {dep_repo.name} already at correct version")
                    
                    success_count += 1
                else:
                    console.print(f"  [yellow]?[/yellow] No updates needed for {dep_repo.name}")
                    success_count += 1  # Consider this successful
                    
            except Exception as e:
                console.print(f"  [red]✗[/red] Failed to update {dep_repo.name}: {e}")
                logger.error(f"Failed to update dependent repository {dep_repo.name}: {e}")
        
        if success_count == len(ordered_dependents):
            console.print(f"[green]✓ Successfully updated all {len(ordered_dependents)} dependent repositories[/green]")
            return True
        else:
            console.print(f"[yellow]Updated {success_count}/{len(ordered_dependents)} dependent repositories[/yellow]")
            return False
    
    def _find_all_dependent_repositories(self, released_repos: List[RepositoryConfig]) -> List[RepositoryConfig]:
    ...
    def _update_dependency_version(self, dependent_repo: RepositoryConfig, 
                                 package_name: str, version: str) -> bool:
        ...
        try:
            ...
            if found_dependency is None:
                logger.info(f"Dependency {package_name} (or variations) not found in {dependent_repo.name}")
                return False
            
            logger.info(f"Found dependency '{actual_dep_name}' in {dependent_repo.name}")
            ...
            if isinstance(found_dependency, str):
                ...
                if 'version' in found_dependency:
                    ...
                    logger.info(f"Skipping version update for local path dependency {actual_dep_name} in {dependent_repo.name}")
                    return False
                else:
            ...
            if changed:
                ...
                logger.info(f"Updated {actual_dep_name} from {original_dependency} to {new_version_spec} in {dependent_repo.name}")
                return True
            else:
                logger.info(f"Dependency {actual_dep_name} in {dependent_repo.name} already at version {original_dependency}")
                return False
                
        except Exception as e:
            logger.error(f"Failed to update dependency {package_name} in {dependent_repo.name}: {e}")
            return False
    ...
    def _calculate_dependent_version_bump(self, current_version: str) -> str:
        ...
        if not match:
            ...
            logger.warning(f"Could not parse version {current_version}, appending -alpha.1")
        ...
        for repo_name, backup in self.backups.items():
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(f"Standard lock failed during rollback for {repo.name}, trying --regenerate")
                    subprocess.run(
                        ["poetry", "lock", "--regenerate"],
                        cwd=repo.path,
                        check=True,
    ...
    def _cleanup_orphaned_tags(self, repo: RepositoryConfig, backup_commit: str) -> None:
    ...
    def _get_last_release_info(self, repo: RepositoryConfig) -> Optional[Dict[str, str]]:
